...
public class CartsManager {
    ...
    public Cart getCart(String userEmail) {
        ...
        if(!cartDTO.getRestaurantId().isEmpty()) {
            restaurant = RestaurantMapper.getInstance().find(cartDTO.getRestaurantId()).getRestaurantForm();
        }
    ...
    public Response<Void> addToCart(String userEmail, String restaurantId, String foodName, int quantity, boolean isFoodPartyFood) {
        ...
        if(restaurant == null) {
            ...
            if(cart.isEmpty() || cart.getRestaurantId().equals(restaurantId)) {
                ...
                return new Response<>("Your desired food was successfully added to your cart!", true);
            }
            else {
                return new Response<>("You have other food from other restaurants in your cart!", false);
            }
        }
    }

    public Response<Void> deleteFromCart(String userEmail, String restaurantId, String foodName, boolean isFoodPartyFood) {
        Restaurant restaurant = RestaurantsManager.getInstance().getRestaurantById(restaurantId);
        if(restaurant == null) {
            return new Response<>("Restaurant doesn't exist!", false);
        }
    ...
    public void decreaseItemQuantity(String userEmail, String foodName, String restaurantId) {
        ...
        if(cartItem.getQuantity() == 1) {
            ...
            ArrayList<CartItemDTO> cartItemDTOs = CartItemMapper.getInstance().findAll(userEmail, null, null);
